...
vector<unsigned char> *SHA256ex::GenerateStretchedData(
    vector<unsigned char> originalMessage,
    int keylength,
    unsigned char *hash,
    vector<unsigned char> added,
    vector<unsigned char> postfix,
    unsigned char **newSig)
{
  /* ---- build the forged message (without postfix) ---- */
  vector<unsigned char> *ret = new vector<unsigned char>();
  for (unsigned char c : originalMessage)
    ret->push_back(c);

  /* padding as in the old function */
  int tailLenBits = (ret->size() + keylength) * 8;
  ret->push_back(0x80);
  while ((ret->size() + keylength + 8) % 64 != 0)
    ret->push_back(0x00);
  for (int i = 0; i < 4; ++i)
    ret->push_back(0x00);
  ret->push_back((tailLenBits >> 24) & 0xFF);
  ret->push_back((tailLenBits >> 16) & 0xFF);
  ret->push_back((tailLenBits >> 8) & 0xFF);
  ret->push_back((tailLenBits) & 0xFF);

  /* ---- prepare context with previous state ---- */
  SHA256_CTX ctx;
  SHA256_Init(&ctx);
  ctx.Nl = (ret->size() + keylength) * 8;
  ctx.h[0] = hash[3] | (hash[2] << 8) | (hash[1] << 16) | (hash[0] << 24);
  ctx.h[1] = hash[7] | (hash[6] << 8) | (hash[5] << 16) | (hash[4] << 24);
  ctx.h[2] = hash[11] | (hash[10] << 8) | (hash[9] << 16) | (hash[8] << 24);
  ctx.h[3] = hash[15] | (hash[14] << 8) | (hash[13] << 16) | (hash[12] << 24);
  ctx.h[4] = hash[19] | (hash[18] << 8) | (hash[17] << 16) | (hash[16] << 24);
  ctx.h[5] = hash[23] | (hash[22] << 8) | (hash[21] << 16) | (hash[20] << 24);
  ctx.h[6] = hash[27] | (hash[26] << 8) | (hash[25] << 16) | (hash[24] << 24);
  ctx.h[7] = hash[31] | (hash[30] << 8) | (hash[29] << 16) | (hash[28] << 24);

  /* ---- hash attacker‑controlled data ---- */
  if (!added.empty())
    SHA256_Update(&ctx, added.data(), added.size());

  /* ---- hash postfix (server‑side suffix) ---- */
  if (!postfix.empty())
    SHA256_Update(&ctx, postfix.data(), postfix.size());

  *newSig = new unsigned char[32];
  SHA256_Final(*newSig, &ctx);

  /* append ONLY attacker data to the message we must send */
  for (unsigned char c : added)
    ret->push_back(c);
