...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    static struct option long_options[] = {
        ...
        {"postfix", required_argument, 0, 0},
        {"help", no_argument, 0, 0},
        {0, 0, 0, 0}};

    int c = getopt_long(argc, argv, "ts:d:a:k:p:h", long_options, &option_index);
    if (c == -1)
      break;
  ...
  vector<unsigned char> vpost = StringToVector((unsigned char *)postfix.c_str());

  unsigned char firstSig[128];
  DigestToRaw(sig, firstSig);
  unsigned char *secondSig;
  vector<unsigned char> *secondMessage;

  if (auto sx = dynamic_cast<SHA256ex *>(sex))
  {
    secondMessage = sx->GenerateStretchedData(vmessage, keylength, firstSig, vtoadd, vpost, &secondSig);
  }
  else
  {
